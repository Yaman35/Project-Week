...
addBtn.addEventListener("click", function () {          // + BUTONUNA basıldığında 
    ...
    li.innerHTML = input.value + '<div><button class="completed-item"><i class="far fa-check-circle"></i></button>    <button class="delete-item"><i class="fas fa-trash-alt"></i></button></div>'; // Takiben bu li etiketinin içeriğine de hem input tan gelecek değeri hem de bir adet çöp kutusu iconu ekledik
...
ul.addEventListener("click", (event) => {               // ul etiketi üzerine tıklandığında
    if (event.target.className === "fas fa-trash-alt") { // Eğer kullanıcının tık hedefinin classı "fas fa-trash-alt" ise, (yani çöp kutusu)
        document.querySelector("li").remove(); // Kullanıcının tıkladığının parentının(button) parantının(li) classına "delete-task" classını ekle ki
